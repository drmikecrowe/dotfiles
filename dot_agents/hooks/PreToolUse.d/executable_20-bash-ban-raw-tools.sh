...
case "$FIRST" in
  cat|head|tail) echo "BLOCKED '$FIRST'. Use the Read tool, or 'rtk $FIRST ...' if a shell pipeline forces it." >&2 ;;
  find) echo "BLOCKED 'find'. Use the Glob tool, or 'rtk fd ...' if a shell pipeline forces it." >&2 ;;
  grep) echo "BLOCKED 'grep'. Use the Grep tool, or 'rtk rg ...' if a shell pipeline forces it." >&2 ;;
  rg) echo "BLOCKED bare 'rg'. Must be 'rtk rg ...' — rtk wraps output through compression hooks. Bare rg floods context. (Or use the Grep tool from the agent.)" >&2 ;;
