...
curl -sf --connect-timeout 2 -X POST http://127.0.0.1:8765/mcp \
  ...
  log_op "SKIP Serena not running"
...
if [ -n "$ACTIVATED" ]; then
  ...
  log_op "activated: $PROJECT_NAME"
  echo "Serena activated: $PROJECT_NAME"
else
  log_op "activation sent for: $REPO_ROOT"
